...
class GRUCell(nn.Module):
    ...
    def forward(
        ...
        H = self.hidden_size
        input_r, input_z, input_n = igates[:, :H], igates[:, H:2*H], igates[:, 2*H:]
        hidden_r, hidden_z, hidden_n = hgates[:, :H], hgates[:, H:2*H], hgates[:, 2*H:]
...
class _GRULayer(nn.Module):
    ...
    def forward(
        ...
        if self.bidirectional:
            hx_bw_val = hx_bw if hidden is not None else None
            result_bw, h_bw = self.layer_bw(x, hx_bw_val, reverse=True)
...
class GRU(nn.Module):
    ...
    def __init__(
        ...
        layers: List[_GRULayer] = [
            _GRULayer(
                input_size, hidden_size, bias, batch_first=False,
                bidirectional=bidirectional, **factory_kwargs,
            )
        ]
        for _ in range(1, num_layers):
            layers.append(
                _GRULayer(
                    hidden_size * num_directions, hidden_size, bias, batch_first=False,
                    bidirectional=bidirectional, **factory_kwargs,
                )
            )
    ...
    def from_float(cls, other, qconfig=None):
        ...
        observed = cls(
            other.input_size, other.hidden_size, other.num_layers, other.bias,
            other.batch_first, other.dropout, other.bidirectional,
        )
