...
class GRUCell(nn.Module):
    ...
    def __init__(
        ...
        self.one_minus_z = torch.ao.nn.quantized.FloatFunctional()
    ...
    def forward(self, x: Tensor, hidden: Optional[Tensor] = None) -> Tensor:
        ...
        one_minus_z_t = self.one_minus_z.add_scalar(
            self.one_minus_z.mul_scalar(z_t, -1.0), 1.0
        )
        h_t = self.add_h.add(
            self.mul_1mz_n.mul(one_minus_z_t, n_t),
    ...
    def from_params(
        ...
        cell.input_linear.weight = nn.Parameter(wi.detach().clone())
        if bi is not None:
            cell.input_linear.bias = nn.Parameter(bi.detach().clone())
        cell.hidden_linear.weight = nn.Parameter(wh.detach().clone())
        if bh is not None:
            cell.hidden_linear.bias = nn.Parameter(bh.detach().clone())
        return cell
...
class _GRULayer(nn.Module):
    ...
    def from_float(cls, other, layer_idx=0, qconfig=None, **kwargs):
        qconfig = getattr(other, "qconfig", None) or qconfig
        ...
        input_size = kwargs.get("input_size", wi.shape[1])
...
Converts a standard nn.GRU into observable form with nn.Linear +
    ...
    def __init__(
        ...
        if self.dropout > 0 and num_layers == 1:
            warnings.warn(
                "dropout is applied between recurrent layers, so non-zero dropout "
    ...
    def forward(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            h_list.append(h)
            if self.dropout > 0 and idx < self.num_layers - 1:
