...
_PERMUTE_TARGET_KEYS = frozenset(
    {
        target_key(exir_ops.edge.aten.permute_copy.default),
        target_key(torch.ops.aten.permute.default),
    }
)
_VIEW_TARGET_KEYS = frozenset(
    {
        target_key(exir_ops.edge.aten.view_copy.default),
        target_key(exir_ops.edge.aten.select.int),
        target_key(exir_ops.edge.aten.unsqueeze_copy.default),
        target_key(exir_ops.edge.aten.squeeze_copy.dims),
        target_key(exir_ops.edge.aten.select_copy.int),
        target_key(torch.ops.aten.reshape.default),
        target_key(torch.ops.aten.flatten.using_ints),
    }
)
_CAST_TARGET_KEYS = frozenset(
    {
        target_key(exir_ops.edge.dim_order_ops._to_dim_order_copy.default),
        target_key(torch.ops.aten._to_copy.default),
    }
)
...
class EthosU55DtypeSupport(OperatorSupportBase):
    ...
    def is_node_supported(  # noqa: C901
        ...
        if target_key(node.target) == _CONVOLUTION_KEY:
...
class EthosU55NotSupported(OperatorSupportBase):
    ...
    def is_node_supported(
        ...
        """
        unsupported_target_keys = frozenset(target_key(op) for op in self.unsupported_ops)
        if target_key(node.target) in unsupported_target_keys:
...
def _memory_format_differs(shape: shape_t, spatial_rank: int) -> bool:
    if len(shape) <= 2 or spatial_rank <= 0:
        return False
    channel_idx = _channel_index(shape, spatial_rank)
    spatial_dims = shape[channel_idx + 1 :]
    if not spatial_dims:
        return False
    return shape[channel_idx] > 1 and any(dim > 1 for dim in spatial_dims)


def _channel_index(shape: shape_t, spatial_rank: int) -> int:
    if len(shape) <= 2:
        return len(shape) - 1
...
def _batch_product(shape: shape_t, spatial_rank: int) -> int:
    product = 1
    for dim in shape[: _channel_index(shape, spatial_rank)]:
        product *= dim
    return product


def _is_channel_reshape(
    input_shape: shape_t,
    output_shape: shape_t,
    input_spatial_rank: int,
    output_spatial_rank: int,
) -> bool:
    valid_ranks = {4, 5, 6}
    if not (len(input_shape) in valid_ranks and len(output_shape) in valid_ranks):
        return False
    old_c = input_shape[_channel_index(input_shape, input_spatial_rank)]
    new_c = output_shape[_channel_index(output_shape, output_spatial_rank)]
    old_n = _batch_product(input_shape, input_spatial_rank)
    new_n = _batch_product(output_shape, output_spatial_rank)
    return old_c != new_c or old_n != new_n


def _permutation_vector_to_matrix(permutation_vector: list[int]) -> torch.Tensor:
    size = len(permutation_vector)
...
def _permutation_matrix_to_vector(permutation_matrix: torch.Tensor) -> list[int]:
    vector = [0] * len(permutation_matrix)
    for row_index, row in enumerate(permutation_matrix):
        for col_index, value in enumerate(row):
            if value == 1:
                vector[row_index] = col_index
                break
    return vector


def _transform_permutation_vector(
    permutation_vector: list[int], dim_order: list[int]
) -> list[int]:
    dim_order_matrix = _permutation_vector_to_matrix(dim_order)
    permutation_matrix = _permutation_vector_to_matrix(permutation_vector)
    transformation_matrix = dim_order_matrix.matmul(
        permutation_matrix.matmul(dim_order_matrix.t())
    )
    return _permutation_matrix_to_vector(transformation_matrix)


def _tosa_shape(shape: shape_t, dim_order: list[int]) -> shape_t:
    reordered = [shape[dim] for dim in dim_order]
    return [-1 if isinstance(dim, torch.SymInt) else dim for dim in reordered]
...
class EthosU55ViewCheck(OperatorSupportBase):
    ...
    def _transpose_requirements(
        ...
        if input_rank > 4 and output_rank <= 4:
            ...
            return (
                True,
                output_rank == 4
                and _memory_format_differs(output_shape, self._spatial_rank(output_rank)),
            )
        elif input_rank <= 4 and output_rank > 4:
            # Assume output needs transpose if going from low-rank to high-rank
            return (
                input_rank == 4
                and _memory_format_differs(input_shape, self._spatial_rank(input_rank)),
                True,
            )
    ...
    def is_node_supported(
        ...
        if target_key(node.target) not in _VIEW_TARGET_KEYS:
            return True
...
class EthosU55TransposeCheck(OperatorSupportBase):
    ...
    def is_node_supported(
        ...
        """
        if target_key(node.target) not in _PERMUTE_TARGET_KEYS:
            return True
        ...
        if rank == 3 or rank == 4:
            ...
            nhwc_shape = _tosa_shape(shape, [0, 2, 3, 1])
            nhwc_permutation = _transform_permutation_vector(permutation, [0, 2, 3, 1])
...
class EthosU55CastCheck(OperatorSupportBase):
    ...
    def is_node_supported(
        ...
        """
        if target_key(node.target) not in _CAST_TARGET_KEYS:
            return True
