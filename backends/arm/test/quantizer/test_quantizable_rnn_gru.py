...
class TestQuantizableRecurrentConversion:
    ...
    def test_from_float_honors_explicit_qconfig(
        self, float_cls, quantizable_cls
    ):
        source = float_cls(4, 6).eval()
        source.qconfig = None
        ...
        converted = quantizable_cls.from_float(source, qconfig=qconfig)
        cell = converted.layers[0].layer_fw.cell

        assert converted.qconfig is not None
        assert converted.layers[0].qconfig is not None
        assert isinstance(
            cell.input_linear.activation_post_process,
            type(qconfig.activation()),
        )
