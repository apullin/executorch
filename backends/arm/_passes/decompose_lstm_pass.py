...
class DecomposeLstmPass(ArmPass):
    ...
    def _build_direction(
        ...
        for t_idx in time_indices:
            ...
            i_t = create_node(
                graph, self._sigmoid, args=(i_pre,), from_node=node
            )
            f_t = create_node(
                graph, self._sigmoid, args=(f_pre,), from_node=node
            )
            g_t = create_node(
                graph, self._tanh, args=(g_pre,), from_node=node
            )
            o_t = create_node(
                graph, self._sigmoid, args=(o_pre,), from_node=node
            )

            # c_t = f_t * c_{t-1} + i_t * g_t
            f_c = create_node(
                graph, self._mul, args=(f_t, c_prev), from_node=node
            )
            i_g = create_node(
                graph, self._mul, args=(i_t, g_t), from_node=node
            )
            c_t = create_node(
                graph, self._add, args=(f_c, i_g), from_node=node
            )

            # h_t = o_t * tanh(c_t)
            tanh_c = create_node(
                graph, self._tanh, args=(c_t,), from_node=node
            )
            h_t = create_node(
                graph, self._mul, args=(o_t, tanh_c), from_node=node
            )
        ...
        for node in list(graph.nodes):
            ...
            with graph.inserting_before(node):
                ...
                for layer_idx in range(num_layers):
                    ...
                    fw_h0 = create_node(
                        graph, self._select,
                        args=(h0_node, 0, h_idx), from_node=node,
                    )
                    fw_c0 = create_node(
                        graph, self._select,
                        args=(c0_node, 0, h_idx), from_node=node,
                    )

                    fw_outputs, fw_h_final, fw_c_final = self._build_direction(
                        graph, node, current_input,
                        fw_h0, fw_c0,
                        fw_w_ih, fw_w_hh, fw_b_ih, fw_b_hh,
                        hidden_size, seq_len, time_dim, reverse=False,
                    )
                    ...
                    if bidirectional:
                        ...
                        bw_outputs, bw_h_final, bw_c_final = self._build_direction(
                            graph, node, current_input,
                            bw_h0, bw_c0,
                            bw_w_ih, bw_w_hh, bw_b_ih, bw_b_hh,
                            hidden_size, seq_len, time_dim, reverse=True,
                        )
                        ...
                        layer_output = create_node(
                            graph, self._cat,
                            args=(merged, time_dim), from_node=node,
                        )

                        layer_final_hiddens.append(create_node(
                            graph, self._unsqueeze,
                            args=(fw_h_final, 0), from_node=node,
                        ))
                        layer_final_hiddens.append(create_node(
                            graph, self._unsqueeze,
                            args=(bw_h_final, 0), from_node=node,
                        ))
                        layer_final_cells.append(create_node(
                            graph, self._unsqueeze,
                            args=(fw_c_final, 0), from_node=node,
                        ))
                        layer_final_cells.append(create_node(
                            graph, self._unsqueeze,
                            args=(bw_c_final, 0), from_node=node,
                        ))
                    else:
                        layer_output = create_node(
                            graph, self._cat,
                            args=(fw_outputs, time_dim), from_node=node,
                        )
                ...
                if len(layer_final_hiddens) == 1:
                    ...
                    h_n = create_node(
                        graph, self._cat,
                        args=(layer_final_hiddens, 0), from_node=node,
                    )
                if len(layer_final_cells) == 1:
                    ...
                    c_n = create_node(
                        graph, self._cat,
                        args=(layer_final_cells, 0), from_node=node,
                    )
